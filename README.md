...
*   **🍎 极简 UI 设计**: 采用 Apple 风格的深色模式界面，纯黑背景 + 系统蓝配色，清晰护眼。
*   **📂 智能下载管理**:
    *   自动按公众号名称创建文件夹。
    *   支持断点续传，自动跳过已下载文章。
...
### 2. 下载源码
```bash
git clone https://github.com/your-username/wechat-article-downloader.git
...
3.  输入 **公众号名称** (例如 "薪火传")。
4.  选择需要的导出格式 (PDF/Word)。
5.  点击 **“⚡ 开始下载”**。

## ⚠️ 注意事项

*   **账号要求**: 需要有微信公众号平台的登录权限（个人订阅号即可）。
*   **频率限制**: 微信接口有频率限制，如果程序提示“触发频率限制”，请耐心等待 60 秒或更换账号。
*   **PDF 生成**: PDF 生成依赖本地 Chrome 浏览器，速度可能稍慢，请耐心等待。
